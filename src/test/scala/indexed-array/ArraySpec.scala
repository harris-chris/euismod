...
class ArraySpec extends AnyFeatureSpec with GivenWhenThen with Matchers {
  ...
  feature("The TransAllDT typeclass") {
    ...
    scenario("transposing a 3d array with AllSlice correctly flips the axes") {
    ...
    scenario("transposing a 4d array with AllSlice correctly flips the axes", TransAllDTTest) {
  ...
  feature("The FromElemsDT typeclass") {
    ...
    scenario("a List3d can be constructed from a list of Ts", FromElemsDTTest) {
      ...
      assert(actO match {
        case Some(act) => {
          println(PrettyPrint[List3d, Double].apply(act))
          println("ACTUAL")
          println(act)
          act === exp
        }
